...
import type { ExtendedPluginTab } from "../types";
...
export default class PluginCommands {
        ...
        },
        {
            ...
            callback: () => {
                ...
                gfs.display((results) => {
                    ...
                    this.plugin.app.plugins.disablePluginAndSave(results.info as string);
                });
            }
        },
        {
    ...
    public ribbonDisplayCommands(): void {
        ...
        const gfs = new GenericFuzzySuggester(this.plugin);
        const settings = this.plugin.app.setting;
        const listOfCoreSettingsTabs: SuggesterItem[] = Object.values(settings.settingTabs).map(
            (t: ExtendedPluginTab) => {
                return {
                    display: "Core: " + t.name,
                    info: () => {
        ...
        const listOfPluginSettingsTabs: SuggesterItem[] = Object.values(settings.pluginTabs).map(
            (t: ExtendedPluginTab) => {
                return {
